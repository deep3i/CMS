...
const CreateFormComponent = ({ handleBack, edit, editDetails, setEditDetails, setEdit }) => {
  ...
  useEffect(() => {
    if (edit && Object.entries(editDetails)?.length > 0) {
      const { title: blogTitle, BlogManage: { status }, categories: editCat } = editDetails;
      setTitle(blogTitle);
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      if (edit) {
        ...
        delete Obj.content;
        dispatch(updateBlogPostAsync({ data: Obj, status: selectedState, callback: handleCallback }));
      } else {
        dispatch(createBlogPostAsync({ data: Obj, status: selectedState, callback: handleCallback }));
      }
    } catch (error) {
  ...
  return (
    <div className="flex justify-center max-w-7xl">
      <form
        className={`w-full p-4 rounded shadow-md bg-gray-900`}
        onSubmit={handleFormSubmit}
      >
        <h2
          className={`text-xl mb-4 tracking-wider font-lighter text-${styles.primary}`}
        >
        ...
        <div className="mb-4">
          ...
          >
            <option selected>
              {translations?.breadcrumb?.posts?.chooseState}
            </option>
