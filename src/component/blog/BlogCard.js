...
const BlogCardComponent = () => {
  ...
  const returnCategoriesName = (id) => {
    return (
      categories?.length > 0 &&
      categories.filter((cat) => String(cat.id) === id)[0]?.name
    );
  };
  ...
  return (
    <>
      <section
        className={`py-6 sm:py-12 dark:bg-${styles.main} dark:text-${styles.primary}`}
      >
        <div className="container p-6 mx-auto space-y-8">
          <div className="space-y-2 text-center">
            ...
            <p className={`font-serif text-sm dark:text-${styles.primary}`}>
              Experience is the best teacher. But sometimes, the tuition is just
              too high. Smart people learn from other's mistakes. They also
              learn from other's success.
            </p>
            <p className={`font-serif text-sm dark:text-${styles.primary}`}>
              These titles speak to the problems your target personas are facing
              and promise to deliver insight on how to deal with these problems.
            </p>
          </div>
              ...
              blogPosts.map(
                (item) =>
                  isBlogMatchedCategories(item.categories) && (
                    <article
                      key={item.id}
                      className={`flex flex-col p-2 bg-${styles.main}`}
                    >
