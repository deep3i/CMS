...
const TableComponent = ({ userRole = 'user', rowData, translations, callAPI, setCallAPI }) => {
  ...
  const { user: { access } } = useSelector(getAllUserDetail);
                ...
                <tr key={index} className="bg-white border-4 border-gray-200">
                  <td className="px-16 py-2 flex flex-row">
                    <div className="relative">
                      <img
                        ...
                        alt={row.img}
                      />
                      <span className={`top-0 left-7 absolute  w-3.5 h-3.5 ${row.status === "active" ? 'bg-green-400' : 'bg-red-400'} border-2 border-white dark:border-gray-800 rounded-full`}></span>
                    </div>
                  </td>
                  ...
                  >
                    ...
                    {row?.status === "banned" && handleCondition(row.role) ? (
                      <span data-tooltip-id="restore-tooltip" className={`${handleAccess('restore') ? '' : 'hidden'}`}>
                        <FontAwesomeIcon
                          cursor="pointer"
                      ...
                      <span data-tooltip-id="delete-tooltip" className={`${handleAccess('delete') ? '' : 'hidden'}`}>
                        <FontAwesomeIcon
                          cursor="pointer"
                          onClick={() => handleLogut(row, "other")}
                          icon={faTrash}
